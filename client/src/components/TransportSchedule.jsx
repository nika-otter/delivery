...
const AddTransportSchedule = () => {
  ...
  const getTransportLocation = (transportId) => {
    const transport = transports.find(
      (transport) => transport.id == transportId
    );
    return transport ? transport.location_id : "";
  };
        ...
        <div className="py-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {console.log("gates", gates[0].location_id)} */}
                {console.log("locations", selectedTransport)}
                {gates
                  .filter(
                    (gate) =>
                      gate.location_id ==
                      getTransportLocation(selectedTransport)
                  )
                  .map((gate) => (
